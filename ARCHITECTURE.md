...
- **`scan` is two-phase; findings are always complete.** `scan(client, &[CompactBlock], &keys, &network) -> Result<Transactions>`: phase 1 = `scan_compact` (sans-IO, parallel — compact-decrypt → receives *without* memos, + spends); phase 2 = fetch each owning full tx (`chain`) and full-decrypt the matching output (`note`) to recover the memo. A failed fetch errors the whole chunk. `scan_compact` is exposed alone (bench / offline). Memo recovery is **not** "enrichment": the memo is the tail of the full note plaintext, so it falls out of full decryption; compact blocks merely truncate it.

- **Findings model.** `Transactions { orchard: Vec<Tx<orchard::Note, orchard::Address>>, sapling: Vec<Tx<…>> }` — per-pool vecs (pool is a *type* axis). `Tx<N,A>` = `Receive(Receive<N,A>)` | `Spend(Spend)`; `Spend` is non-generic. `Tx::{height, txid, nf}` expose the nullifier join key uniformly.

- **One inline loop — no `PassOutcome`.** `sync::run(client, keys, network, resume_point, rewind, sink)` handles transport faults (reconnect + resume) and reorgs (the consumer's `rewind`, doubling walk-back) **inline**, returning `Ok(())` once synced. No outcome enum crosses the boundary. Persistence-free: consumer state is reached only through the three closures.

- **The sink carries the seam hash.** `sink(scanned_height, hash, &Transactions)`: height is the spine, `hash` is reorg seam material the consumer records, findings are the data.

- **db consumer — no `RefCell`.** Every `Db` op is `&self` (`rewind_to_height` uses `Connection::unchecked_transaction`), so `db::sync::sync_to_tip(&Db, client, keys)` shares a plain `&Db` across the three closures. `apply` writes receives, records spends **only when the nullifier matches an owned note**, and advances the cursor; note positions ride on `Receive.position`.
- **The schema is right-sized for an observer — no `blocks` table.** Live testing exposed that the inherited per-block `blocks` table (with `transactions.block → blocks.height`) couldn't be fed by the minimal sink (only the chunk tip crosses), so a spend at a non-tip height blew the FK. An observer tracks notes *by height*, not a block ledger, so the table — and `transactions.block`, the `height_consistency` CHECK, and `sync_state`'s running tree-size columns — are gone. The cursor (`sync_state` height + hash) is the **only** chain-position state; `rewind_to_height` is `DELETE WHERE mined_height > h` + cursor reset (seam hash cleared — resume re-verifies forward).
